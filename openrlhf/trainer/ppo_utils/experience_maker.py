    ...
    def generate_stepped_samples(self, samples: Samples, step_separator: str) -> Samples:
        ...
        if self.packing_samples:
            ...
            for i, num_step in enumerate(num_steps):
                ...
                num_actions.append(max(1, response_len))
                step_positions.append(step_position)

                offset += num_step

            sequences = torch.tensor(sum(sequences, []), device="cuda").unsqueeze(0)
            attention_mask = torch.tensor(attention_mask, device="cuda").unsqueeze(0)
            action_mask = None
            response_length = torch.tensor(num_actions, device="cuda", dtype=torch.float)
            total_length = torch.tensor(packed_seq_lens, device="cuda", dtype=torch.float)

            assert sequences.size(1) == total_length.sum()
            ...
            if isinstance(reward[0], list):
                ...
                for step_reward, step_positions, response_length in zip(
                    ...
                    assert len(step_reward) == len(
                        step_positions
                    ), f"step_reward and step_positions should have same length, {len(step_reward)} != {len(step_positions)}"
                    converted_stepped_reward = torch.zeros((int(response_length),), dtype=torch.float32, device=device)
                    for step_reward, step_position in zip(step_reward, step_positions):
                        converted_stepped_reward[step_position - 1] = step_reward
                    converted_stepped_rewards.append(converted_stepped_reward)
                converted_rewards.append(converted_stepped_rewards)
            else:
                # orm
                converted_rewards.append(torch.tensor(reward, dtype=torch.float32, device=device))
